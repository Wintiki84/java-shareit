...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b FROM  Booking b " +
            ...
            "ORDER BY b.start")
    Booking findLastBooking(LocalDateTime now, Long userId, Long itemId, PageRequest pageRequest);

    @Query("SELECT b FROM  Booking b " +
            "WHERE b.item.id = :itemId AND b.item.owner.id = :userId AND b.status = 'APPROVED' AND b.start > :now " +
            "ORDER BY b.start")
    Booking findNextBooking(LocalDateTime now, Long userId, Long itemId, PageRequest pageRequest);
